...
$(document).ready(() => {
	...
	const update_target = (ev) => {
		...
		for (const target_selector of target_selectors) {
			...
			let url = "{{ site.base }}/" + window.mirrorId
			if (window.mirrorId.endsWith(".git")) {
				url = "{{ site.base }}/git/" + window.mirrorId
			}
			const template_data = {
	...
	$.getJSON("{{ site.base }}/static/tunasync.json", (statusData) => {
		...
		if (!availableMirrorIds.has(window.mirrorId)) {
			if ({{ site.hide_mirrorz }}) {
				location.href = "{{ site.base }}/404-help-hidden.html"; // this will break 404 issue submission
			} else {
