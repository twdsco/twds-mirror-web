...
wget -q "http://127.0.0.1:3000/render/d-solo/ddhj7whaercw0e/mirror-monitor?scale=3&theme=$t&panelId=8&from=now-$p&width="$W"&height="$H -O "$tmp"/"$t"_disku_"$p".png &
wget -q "http://127.0.0.1:3000/render/d-solo/ddhj7whaercw0e/mirror-monitor?scale=3&theme=$t&panelId=9&from=now-$p&width="$W"&height="$H -O "$tmp"/"$t"_diskrw_"$p".png &
for pid in `jobs -p`
do
wait $pid
done
