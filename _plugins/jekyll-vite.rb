...
class Jekyll::Vite::Generator < Jekyll::Generator
  ...
  def generate(site)
    ...
    export_dir = File.join(cache_dir, 'vite-exports')
    FileUtils.mkdir_p(export_dir)
    exports.each do |export|
      path = File.join(export_dir, "#{export[:name]}.json")
      File.write(path, Jekyll_Filters.new.jsonify(export[:data]))
      vr.env[export[:name]] = path
      export[:path] = path
    end
    ...
    exports.each do |export|
      File.delete(export[:path]) if File.exist?(export[:path])
    end
  end
